...
from config import KEY, CHACHA20_NONCE, CFB_IV, OFB_NONCE, CTR_NONCE
...
class LsServer(SecureSocket):
    def __init__(self, loop: asyncio.AbstractEventLoop, listenAddr: net.Address, cipher_type: str) -> None:
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        usage='%(prog)s ip port [--cipher-type {ChaCha20,AES-256-CFB,AES-256-OFB,AES-256-CTR}]',
        description='LS Server - Secure file transfer server'
    )

    parser.add_argument('ip', type=str, help='The IP address of the server')
    parser.add_argument('port', type=int, help='The port number for the server to listen on')
    ...
    listenAddr = net.Address(args.ip, args.port)
    print(f'LS server is starting on {listenAddr} using {args.cipher_type} encryption')

    server = LsServer(loop, listenAddr, args.cipher_type)
